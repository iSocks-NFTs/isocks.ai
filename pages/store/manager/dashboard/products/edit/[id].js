...
const EditProduct = () => {
  const { data, loading, error, id } = useGetProductById();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    validateInput(form);
    const { updateProduct } = adminEndpoints;
    try {
      const response = await axios.patch(updateProduct + id, form, {
        headers: {
      ...
      if (response.status === 200) {
        new Toast(
          "Successfully Updated Product... Proceed to Set Product Image"
        );
      ...
      if (error && error.response) {
        ...
        if (error.response.status === 409) {
          new Toast(
            "There was a problem updating the Product Record on the Server... Please Try Again Later"
          );
        }
      }
  ...
  return (
    <ManagerLayout title="iSocks | Edit Product">
      {!loading ? (
        <>
          {data ? (
...
function ErrorModal() {
  return (
    <div className="w-full h-full bg-black bg-opacity-60 absolute top-0 left-0 flex items-center justify-center">
      <div className="z-40 w-[380px] h-[400px] bg-white shadow-md rounded-md flex flex-col justify-center items-center">
        <h3 className="text-center text-3xl">Oops 😥</h3>
        <p className="text-center">
          There seems to be a problem loading that product
        </p>
        <p className="text-center">Maybe Try Again Later...</p>
      </div>
    </div>
  );
}
